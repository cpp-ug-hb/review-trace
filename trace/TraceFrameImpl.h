...
TraceFrame::TraceFrame()
  : _leader(DeltaTime(0,0))
, _used(0)
{
}

TraceFrame::TraceFrame(const DeltaTimeFW& leader)
: _leader(leader)
, _used(0)
{
}
...
void TraceFrame::erase( size_t pos ) {
  ...
  std::copy(_times.begin()+pos+1, _times.begin()+num_used(), _times.begin()+pos );
  std::copy(_values.begin()+pos+1, _values.begin()+num_used(), _values.begin()+pos );
  --_used;
}
...
void TraceFrame::insert(size_t pos, DeltaTimeFW const& t, Bit const& value) {
  ...
  std::copy_backward(
      _times.begin() + pos
    , _times.begin() + num_used()
    , _times.begin() + num_used() + 1
  );
  std::copy_backward(
      _values.begin() + pos
    , _values.begin() + num_used()
...
bool TraceFrame::full() const {
    return _used == TraceFrameSize;
}
...
DeltaTimeFW&        TraceFrame::time_at ( size_t pos) {
  return _times[pos];
}

DeltaTimeFW const&  TraceFrame::time_at ( size_t pos) const {
  return _times[pos];
}

Bit&              TraceFrame::bit_at  ( size_t pos) {
  return _values[pos];
}

Bit const&        TraceFrame::bit_at  ( size_t pos) const {
  return _values[pos];
}

unsigned          TraceFrame::num_used() const {
  return _used;
}

bool TraceFrame::empty() const {
  return _used == 0;
}
