...
DeltaTime DeltaTime::rebase(DeltaTime const& oldBase, DeltaTime const& newBase) const  {
  if (oldBase == newBase) {
    return *this;
  }
  ...
  if (positiveSide == oldBase._data._simcycle && positiveDelta < oldBase._data._deltacycle ) {
    // underflow
    return DeltaTime(0,0);
  }
